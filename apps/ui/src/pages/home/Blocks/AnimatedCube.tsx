	...
	useFrame((_, delta) => {
		...
		switch (animationState.phase) {
			case 'idle':
				...
				if (isLoading) {
					...
					const waveCycle = (ANIMATION_CONFIG.WAVE_DELAY * (DISPLAY_CONFIG.MAX_CUBES - 1) + 
										ANIMATION_CONFIG.BOUNCE_DURATION + ANIMATION_CONFIG.PAUSE_AFTER_WAVE)
					...
					loadingLiquidOpacityRef.current = Math.max(
						loadingLiquidOpacityRef.current - delta * 3,
						0
					)
				}
				break
			...
			case 'sliding': {
				...
				if (adjustedElapsed < ANIMATION_CONFIG.IMPACT_DURATION) {
					...
					targetScale *= (ANIMATION_CONFIG.IMPACT_SCALE + (1 - ANIMATION_CONFIG.IMPACT_SCALE) * easings.outBack(impactProgress))
				}
				break
			}
		...
		if (animationState.phase === 'entering') {
			...
			if (subPhase === 'spinning-in' || subPhase === 'validating') {
				...
				liquidMaterial.uniforms['uColorProgress'].value = Math.min(liquidMaterial.uniforms['uColorProgress'].value + delta * 3, 1.0)
				...
				<group rotation={[0, Math.PI, 0]}>
					...
					<>
						...
						<Text 
							...
							anchorY="top"
						>
							{displaySubPhase === 'verified' || displaySubPhase === 'spinning-to-front' ? 'Validated' : 'Validating...'}
						</Text>
						<Text 
							position={[-1.2, -1.1, DISPLAY_CONFIG.PLANE_Z + 0.02]} 
							fontSize={0.2} 
