...
export function Scene() {
	...
	useEffect(() => {
		...
		// Check for new block
		const hasNewBlock = prevBlocksRef.current.length > 0 && 
			blocks[0]?.height !== prevBlocksRef.current[0]?.height
	...
	return (
		<>
			...
			{(isLoading 
				? Array.from({ length: DISPLAY_CONFIG.MAX_CUBES })
				: cubes
			).map((cube, index) => {
				...
				return (
					<AnimatedCube
						...
						isLoading={isLoading}
						animationState={isLoading ? { phase: 'idle', startTime: 0 } : (cube as CubeData)?.animationState || { phase: 'idle', startTime: 0 }}
